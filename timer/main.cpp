...
SubMenu::SubMenu(const wxString& title): wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, wxSize(1500, 920))
{
	...
	log_enable = new wxCheckBox(panel, ID_log_enable, wxT("Записывать журнал"));
	log_enable->SetValue(true);
	hbox3->Add(log_enable, 0, wxRIGHT | wxLEFT | wxALIGN_BOTTOM, 10);
	error_control = new wxCheckBox(panel, ID_error_control, wxT("Обработка ошибок"));
	...
	grid->SetColLabelValue(0, wxT("Название сообщения"));
	grid->SetColLabelValue(1, wxT("ID (bin)"));
	grid->SetColLabelValue(2, wxT("Data (hex)"));
	grid->SetColLabelValue(3, wxT("Счетчик"));
	grid->SetColLabelValue(4, wxT("Источник"));
	...
	toggle_launch_stop = new wxToggleButton(panel, ID_toggle_launch_stop, wxT("Пуск"), wxDefaultPosition, wxSize(150, 45));
	...
	clear_grid = new wxButton(panel, ID_clear_grid, wxT("Очистить"), wxDefaultPosition, wxSize(150, 45));
...
void SubMenu::on_toggle_launch_stop(wxCommandEvent& WXUNUSED(event))
{
	if (toggle_launch_stop->GetValue())
	{
		...
		char sTransmitedChar[] = "E0000000A0\r";
		WriteFile(hSerial, sTransmitedChar, strlen(sTransmitedChar), &iSize, 0);
		toggle_launch_stop->SetLabel(wxT("Стоп"));
	...
	else
	{
		...
		WriteFile(hSerial, sTransmitedChar, strlen(sTransmitedChar), &iSize, 0);
		toggle_launch_stop->SetLabel(wxT("Пуск"));
	...
	while (i < iSize)
	{
		if (i + 9 < iSize)
		{
			...
			if (i + 9 + data_size * 2 < iSize)
			{
				...
				if ((ID & mask_triple) == bloader_erase_rslt_ROM || (ID & mask_triple) == bloader_erase_rslt_Flash) //Результат стирания и Несоотв. полученной инф. текущему процессу (0x1E0 и 0x1E8) 
				{
					if ((ID & mask_quatro) == bloader_data_fl_err) //Ошибка вычитывания (0x1E0E)
					{
						if (pos_bloader_data_fl_err < 0)
						{
							pos_bloader_data_fl_err = messages;
							messages++;
							grid->SetCellValue(pos_bloader_data_fl_err, 0, wxT("Несоотв. полученной инф. текущему процессу"));
						}
						...
						strcpy(log_name, "Несоотв. полученной инф. текущему процессу");
						log_name_counter = counter_bloader_data_fl_err;
						
						id_with_error = true;
					}
					else //Результат стирания (0x1E0 и 0x1E8)
					{
						if (pos_bloader_erase_rslt < 0)
						{
							pos_bloader_erase_rslt = messages;
							messages++;
							grid->SetCellValue(pos_bloader_erase_rslt, 0, wxT("Результат стирания"));
						}
						...
						strcpy(log_name, "Результат стирания");
				...
				if ((ID & mask_triple) == cpu_request_erase_fl_ROM || (ID & mask_triple) == cpu_request_erase_fl_Flash) //Запрос стирания (0x1C0 и 0x1C8)
				{
					if (pos_cpu_request_erase_fl < 0)
					{
						pos_cpu_request_erase_fl = messages;
						messages++;
						grid->SetCellValue(pos_cpu_request_erase_fl, 0, wxT("Запрос стирания"));
					}
					...
					strcpy(log_name, "Запрос стирания");
					log_name_counter = counter_cpu_request_erase_fl;
				}
				if ((ID & mask_quatro) == cpu_request_prgrm_fl_ROM || (ID & mask_quatro) == cpu_request_prgrm_fl_Flash) //Запрос программирования (0x1C20 и 0x1CA0)
				{
					if (pos_cpu_request_prgrm_fl < 0)
					{
						pos_cpu_request_prgrm_fl = messages;
						messages++;
						grid->SetCellValue(pos_cpu_request_prgrm_fl, 0, wxT("Запрос программирования"));
					}
					...
					strcpy(log_name, "Запрос программирования");
					log_name_counter = counter_cpu_request_prgrm_fl;
				}
				if ((ID & mask_triple) == cpu_request_inf) //Запрос общей служебной информации (0x1D0) 
				{
					if (pos_cpu_request_inf < 0)
					{
						pos_cpu_request_inf = messages;
						messages++;
						grid->SetCellValue(pos_cpu_request_inf, 0, wxT("Запрос общей служебной информации"));
					}
					...
					strcpy(log_name, "Запрос общей служебной информации");
					log_name_counter = counter_cpu_request_inf;
				}
				if ((ID & mask_triple) == bloader_provide_inf) //Предоставление общей служебной информации или Фиксированное состояние шины CAN (0x1F0)
				{
					if (ID == bloader_bus_off) //Фиксированное состояние шины CAN (0x1F00FFFF)
					{
						if (pos_bloader_bus_off < 0)
						{
							pos_bloader_bus_off = messages;
							messages++;
							grid->SetCellValue(pos_bloader_bus_off, 0, wxT("Фиксированное состояние шины CAN"));
						}
						...
						strcpy(log_name, "Фиксированное состояние шины CAN");
						log_name_counter = counter_bloader_bus_off;
					}
					else //Предоставление общей служебной информации
					{
						if (pos_bloader_provide_inf < 0)
						{
							pos_bloader_provide_inf = messages;
							messages++;
							grid->SetCellValue(pos_bloader_provide_inf, 0, wxT("Предоставление общей служебной информации"));
						}
						...
						strcpy(log_name, "Предоставление общей служебной информации");
				...
				if ((ID & mask_quatro) == cpu_send_data_ROM || (ID & mask_quatro) == cpu_send_data_Flash) //Передача пакета данных (0x1C3F и 0x1CBF)
				{
					if (pos_cpu_send_data < 0)
					{
						pos_cpu_send_data = messages;
						messages++;
						grid->SetCellValue(pos_cpu_send_data, 0, wxT("Передача пакета данных"));
					}
					...
					strcpy(log_name, "Передача пакета данных");
					log_name_counter = counter_cpu_send_data;
				}
				if ((ID & mask_quatro) == cpu_end_data_ROM || (ID & mask_quatro) == cpu_end_data_Flash) //Cообщение о передаче пакета данных (0x1C2F и 0x1CAF)
				{
					if (pos_cpu_end_data < 0)
					{
						pos_cpu_end_data = messages;
						messages++;
						grid->SetCellValue(pos_cpu_end_data, 0, wxT("Cообщение о передаче пакета данных"));
					}
					...
					strcpy(log_name, "Cообщение о передаче пакета данных");
					log_name_counter = counter_cpu_end_data;
				}
				if ((ID & mask_triple) == bloader_prgr_result_ROM || (ID & mask_triple) == bloader_prgr_result_Flash) //Запрос пакета данных и Результат программирования (0x1E2 и 0x1EA)
				{
					if ((ID & mask_quatro) == bloader_request_data_ROM || (ID & mask_quatro) == bloader_request_data_Flash) //Запрос пакета данных (0x1E2F и 0x1EAF)
					{
						if (pos_bloader_request_data < 0)
						{
							pos_bloader_request_data = messages;
							messages++;
							grid->SetCellValue(pos_bloader_request_data, 0, wxT("Запрос пакета данных"));
						}
						...
						strcpy(log_name, "Запрос пакета данных");
						log_name_counter = counter_bloader_request_data;
					}
					else //Результат программирования (0x1E2 и 0x1EA)
					{
						if (pos_bloader_prgr_result < 0)
						{
							pos_bloader_prgr_result = messages;
							messages++;
							grid->SetCellValue(pos_bloader_prgr_result, 0, wxT("Результат программирования"));
						}
						...
						strcpy(log_name, "Результат программирования");
				...
				if ((ID & mask_quatro) == cpu_check_request_ROM || (ID & mask_quatro) == cpu_check_request_Flash) //Запрос сравнения (0x1C40 и 0x1CC0)
				{
					if (pos_cpu_check_request < 0)
					{
						pos_cpu_check_request = messages;
						messages++;
						grid->SetCellValue(pos_cpu_check_request, 0, wxT("Запрос сравнения"));
					}
					...
					strcpy(log_name, "Запрос сравнения");
					log_name_counter = counter_cpu_check_request;
				}
				if ((ID & mask_quatro) == cpu_transmit_data_smpl_ROM || (ID & mask_quatro) == cpu_transmit_data_smpl_Flash) //Передача пакета данных образца (0x1C5F и 0x1CDF)
				{
					if (pos_cpu_transmit_data_smpl < 0)
					{
						pos_cpu_transmit_data_smpl = messages;
						messages++;
						grid->SetCellValue(pos_cpu_transmit_data_smpl, 0, wxT("Передача пакета данных образца"));
					}
					...
					strcpy(log_name, "Передача пакета данных образца");
					log_name_counter = counter_cpu_transmit_data_smpl;
				}
				if ((ID & mask_quatro) == cpu_end_data_smpl_ROM || (ID & mask_quatro) == cpu_end_data_smpl_Flash) //Сообщение о передаче пакета данных образца (0x1C4F и 0x1CCF)
				{
					if (pos_cpu_end_data_smpl < 0)
					{
						pos_cpu_end_data_smpl = messages;
						messages++;
						grid->SetCellValue(pos_cpu_end_data_smpl, 0, wxT("Сообщение о передаче пакета данных образца"));
					}
					...
					strcpy(log_name, "Сообщение о передаче пакета данных образца");
					log_name_counter = counter_cpu_end_data_smpl;
				}
				if ((ID & mask_triple) == bloader_check_result_ROM || (ID & mask_triple) == bloader_check_result_Flash) //Результат сравнения и Запрос пакета данных образца (0x1E4 и 0x1EC)
				{
					if ((ID & mask_quatro) == bloader_request_data_smpl_ROM || (ID & mask_quatro) == bloader_request_data_smpl_Flash) //Запрос пакета данных образца (0x1E4F и 0x1ECF)
					{
						if (pos_bloader_request_data_smpl < 0)
						{
							pos_bloader_request_data_smpl = messages;
							messages++;
							grid->SetCellValue(pos_bloader_request_data_smpl, 0, wxT("Запрос пакета данных образца"));
						}
						...
						strcpy(log_name, "Запрос пакета данных образца");
						log_name_counter = counter_bloader_request_data_smpl;
					}
					else //Результат сравнения (0x1E4 и 0x1EC)
					{
						if (pos_bloader_check_result < 0)
						{
							pos_bloader_check_result = messages;
							messages++;
							grid->SetCellValue(pos_bloader_check_result, 0, wxT("Результат сравнения"));
						}
						...
						strcpy(log_name, "Результат сравнения");
				...
				if ((ID & mask_triple) == cpu_transfer_ctrl_request) //Запрос передачи управления штатному ПО (0x1C6)
				{
					if (pos_cpu_transfer_ctrl_request < 0)
					{
						pos_cpu_transfer_ctrl_request = messages;
						messages++;
						grid->SetCellValue(pos_cpu_transfer_ctrl_request, 0, wxT("Запрос передачи управления штатному ПО"));
					}
					...
					strcpy(log_name, "Запрос передачи управления штатному ПО");
					log_name_counter = counter_cpu_transfer_ctrl_request;
				}
				if ((ID & mask_triple) == bloader_tr_ctrl_confirm) //Подтверждение запроса передачи управления (0x1E6)
				{
					if (pos_bloader_tr_ctrl_confirm < 0)
					{
						pos_bloader_tr_ctrl_confirm = messages;
						messages++;
						grid->SetCellValue(pos_bloader_tr_ctrl_confirm, 0, wxT("Подтверждение запроса передачи управления"));
					}
					...
					strcpy(log_name, "Подтверждение запроса передачи управления");
					log_name_counter = counter_bloader_tr_ctrl_confirm;
				}
				if ((ID & mask_quatro) == cpu_request_read_fl) //Запрос вычитывания информации (0x1CEF)
				{
					if (pos_cpu_request_read_fl < 0)
					{
						pos_cpu_request_read_fl = messages;
						messages++;
						grid->SetCellValue(pos_cpu_request_read_fl, 0, wxT("Запрос вычитывания информации"));
					}
					...
					strcpy(log_name, "Запрос вычитывания информации");
					log_name_counter = counter_cpu_request_read_fl;
				}
				if ((ID & mask_quatro) == bloader_send_data_fl) //Передача пакета данных (0x1EFF)
				{
					if (pos_bloader_send_data_fl < 0)
					{
						pos_bloader_send_data_fl = messages;
						messages++;
						grid->SetCellValue(pos_bloader_send_data_fl, 0, wxT("Передача пакета данных"));
					}
				...
				if ((ID & mask_quatro) == cpu_end_data_fl) //Окончание вычитывания информации (0x1CE0)
				{
					if (pos_cpu_end_data_fl < 0)
					{
						pos_cpu_end_data_fl = messages;
						messages++;
						grid->SetCellValue(pos_cpu_end_data_fl, 0, wxT("Окончание вычитывания информации"));
					}
					...
					strcpy(log_name, "Окончание вычитывания информации");
				...
				if (pos_table >= 0)
				{
					...
					if (buf[3] == '1') //subscriber
						grid->SetCellValue(pos_table, 4, wxString(wxT("Программа-загрузчик")));
					else
						grid->SetCellValue(pos_table, 4, wxString(wxT("Технологический процессор")));
				}
